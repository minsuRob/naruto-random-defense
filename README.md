...
진영 넷이 가운데 광장을 둘러싼다. 광장에는 파쿤이 서 있는 받침대와 제단 넷이
있고, 제단은 상하좌우 네 방향에 하나씩이다. 지금은 내 진영(남서쪽)에만 몹이 돌고,
나머지 셋은 협동용으로 그려만 둔 자리다.
...
1. **초기 지급** — 난이도를 고르면 파쿤과 목재를 받는다(이지 7·19, 하드 9·19,
   헬 11·19). 유닛은 하나도 없다. 준비 시간 25초가 첫 방어선을 세우는 시간이다.
2. **파쿤** — 카운터가 아니라 가운데 광장 받침대 위, 내 진영 쪽 사분면에 실체로
   서 있는 강아지다. 광장의 제단 넷 중 하나로 **보내야** 결과가 나온다. 위는
   노말+매직, 오른쪽은 골드, 아래는 노말, 왼쪽은 목재. Q·W·E·R가 그대로 그 네
   방향이고, 제단을 직접 클릭해도 된다. 라운드가 끝날 때마다 난이도만큼 더 들어온다.
...
| | |
|---|---|
| 파쿤 보내기 | `Q` 위 · `W` 오른쪽 · `E` 아래 · `R` 왼쪽 (제단 클릭도 가능) |
| 카메라 이동 | 방향키 · 화면 가장자리 · 휠클릭 드래그 · 미니맵 클릭 |
| 줌 | 휠 · `-` / `=` |
| 시점 복귀 | `Space` 또는 `H` (내 진영으로) |
| 도움말 | `F1` |
...
실측(시드 6개): 이지 60~66라운드 / 하드 41~60 / 헬 38~53. 초기 지급 목재 19개와
라운드당 파쿤 4개가 원작 수치라 예전보다 열 라운드쯤 더 간다. 난이도 순서는 유지된다.
...
- 협동은 UI와 데이터 구조만 서 있다. 진영 넷이 다 만들어지고 `playerCount`만
  올리면 되지만, 실제 멀티 연결은 없다.
