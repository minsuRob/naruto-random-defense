...
2. **파쿤** — 카운터가 아니라 진영 남쪽에 실체로 서 있는 강아지다. 진영 한가운데
   2×2로 선 제단 넷 중 하나로 **보내야** 결과가 나온다. 왼쪽 위는 노말+매직,
   오른쪽 위는 골드, 왼쪽 아래는 노말, 오른쪽 아래는 목재. Q·W·E·R가 그 배치
   그대로고, 제단을 직접 클릭해도 된다. 라운드가 끝날 때마다 난이도만큼 더 들어온다.
...
| | |
|---|---|
| 파쿤 보내기 | `Q` 왼쪽 위 · `W` 오른쪽 위 · `E` 왼쪽 아래 · `R` 오른쪽 아래 (제단 클릭도 가능) |
