...
제단 4개가 정확히 그 쓸모없는 중앙 칸을 차지하고 있다(`config/map.ts`의 `ALTARS`).
...
- 4인 UI는 `hud/PlayerPanel.tsx`에 슬롯 4개를 그려두고 1번만 채운다. 엔진은 이미
  `players[]`/`plots[]` 배열이라 협동이 붙으면 숫자가 그대로 들어간다.
  단일 플레이어 안정성이 우선이라는 결정.
